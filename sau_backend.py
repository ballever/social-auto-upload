        ...
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            if ids_param:
                ...
                if id_list:
                    ...
                    cursor.execute(
                        f"SELECT * FROM user_info WHERE id IN ({placeholders})", id_list
                    )
                else:
            ...
            print(
                f"\n📋 获取账号信息（{'ids=' + ids_param if ids_param else '全部'}）：{len(rows_list)} 条"
            )
            for row in rows:
                print(row)
...
async def validateAccount():
    ...
    try:
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            return jsonify(
                {"code": 200, "msg": "Account validated successfully", "data": row_dict}
            ), 200

    except Exception as e:
        print(f"验证账号时出错: {str(e)}")
        return jsonify(
            {"code": 500, "msg": f"验证账号失败: {str(e)}", "data": None}
        ), 500
...
def run_async_function(type, id, status_queue):
    ...
    timeout = getattr(conf, "LOGIN_TIMEOUT", 180)
    ...
    match type:
        case "1":
            ...
            try:
                loop.run_until_complete(
                    asyncio.wait_for(
                        xiaohongshu_cookie_gen(id, status_queue), timeout=timeout
                    )
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
                print(f"⚠️ 用户 {id} 的登录流程超时（{timeout}秒），已取消")
            loop.close()
        case "2":
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                loop.run_until_complete(
                    asyncio.wait_for(
                        get_tencent_cookie(id, status_queue), timeout=timeout
                    )
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
        ...
        case "3":
            ...
            try:
                loop.run_until_complete(
                    asyncio.wait_for(
                        douyin_cookie_gen(id, status_queue), timeout=timeout
                    )
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
        ...
        case "4":
            ...
            try:
                loop.run_until_complete(
                    asyncio.wait_for(get_ks_cookie(id, status_queue), timeout=timeout)
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
                print(f"⚠️ 用户 {id} 的登录流程超时（{timeout}秒），已取消")
        ...
        case "5":
            ...
            try:
                loop.run_until_complete(
                    asyncio.wait_for(
                        bilibili_cookie_gen(id, status_queue), timeout=timeout
                    )
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
        ...
        case "6":
            ...
            try:
                loop.run_until_complete(
                    asyncio.wait_for(
                        baijiahao_cookie_gen(id, status_queue), timeout=timeout
                    )
                )
            except asyncio.TimeoutError:
                status_queue.put("timeout")
